...
def test_update_translational_state_zero_acceleration() -> None:
    ...
    assert np.allclose(new_position, expected_position), f"Expected {expected_position}, but got {new_position}"
    assert np.allclose(new_velocity, expected_velocity), f"Expected {expected_velocity}, but got {new_velocity}"
...
def test_update_translational_state_to_vehicle () -> None:
    ...
    assert np.allclose(vehicle_state.position, expected_position), f"Expected {expected_position}, but got {vehicle_state.position}"
    assert np.allclose(vehicle_state.velocity, expected_velocity), f"Expected {expected_velocity}, but got {vehicle_state.velocity}"